...
class AppStore extends Component {
  state = {
    newTabsList: tabsList,
    newAppsList: appsList,
    value: '',
    id: 'SOCIAL',
  }
  ...
  render() {
    const {newTabsList, newAppsList, value, id} = this.state
    let list = []
    list = newAppsList.filter(eachList => {
      const c = eachList.appName
    ...
    return (
      <div className="main_container">
        ...
        <div className="input_element_box">
          <input
        ...
        <ul className="unordered_list">
          {newTabsList.map(eachTab => (
            <TabItem
              key={eachTab.tabId}
              eachTab={eachTab}
              filterApps={this.filterApps}
              id={id}
            />
          ))}
        </ul>
        <ul className="unordered_list_App">
          {listFilter.map(eachApp => (
